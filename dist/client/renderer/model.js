...
export class Model {
  constructor(src, scene, x, y, fullScene = false) {
    _defineProperty(this, "modelLoadedQueue", []);
    ...
    this.loader.load("/3d/" + src, this.modelLoaded.bind(this), this.modelLoadProgress.bind(this), this.modelLoadError.bind(this));
  }
  ...
  modelLoaded(model) {
    ...
    if (this.fullScene) {
      this.mesh = model.scene; // this.model.scene.traverse((obj) => {
      // obj.castShadow = true
      // obj.receiveShadow = true
      // })
    } else {
      this.mesh = model.scene.children[0]; // this.mesh.castShadow = true
  ...
  modelLoadProgress(xhr) {// console.log("Model", this.src, (xhr.loaded / xhr.total) * 100 + "% loaded")
  }
...
export class GridSquare extends Model {
  ...
  enableCoordinates() {
    if (!this.coordinates) {
      ...
      const textMesh = new Mesh(text, new MeshBasicMaterial({
        color: 0xffffff
      }));
      textMesh.position.x = this.pos.x + centerOffsetX;
      textMesh.position.y = this.pos.y + 0.5;
      textMesh.position.z = this.pos.z + centerOffsetZ; // textMesh.rotation.x = Math.Pi
...
_defineProperty(GridSquare, "Colors", {
...
export class AnimatedModel extends Model {
  constructor(src, scene, x, y) {
    ...
    _defineProperty(this, "lastDraw", 0);
  }
