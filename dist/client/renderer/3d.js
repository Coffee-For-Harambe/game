...
import { gridToWorld, WORLD_SCALE } from "./3dutils.js";
import { buildGrid } from "../../shared/gridutils.js";
window.THREE = THREE;
export default class Renderer {
    ...
    if (window.DEBUGGING) {
      ...
      <strong>GAME STATE:</strong>
        <div style="padding-left: 1rem">
          ...
          selectedCharacter: ${this.game.state.selectedCharacter ? this.game.state.selectedCharacter.debugStr() : "null"}<br/>
    ...
    if (this.scenery.fadable) {
      this.scenery.fadable.traverse(obj => {
        if (obj.material) {
          ...
          obj.material.opacity = 1 - Math.pow(this.controls.target.clone().sub(pos.clone()).normalize().dot(dir1), 3);
        }
      });
    }
  ...
  setupCamera() {
    ...
    const camera = new THREE.PerspectiveCamera(16, // FOV
  ...
  setupHDR() {
    ...
    pmremGenerator.compileEquirectangularShader(); // pmremGenerator.compileCubemapShader()

    new RGBELoader().setDataType(THREE.UnsignedByteType).load("/images/lilienstein_1k.hdr", texture => {
  ...
  updateHover(e) {
    ...
    this.raycaster.setFromCamera(mouse, this.camera); // calculate objects intersecting the picking ray
