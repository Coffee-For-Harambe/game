// Code taken from https://github.com/caldwell/renderjson
// Simply converted to ES6 module because of this package's broken compatibility layer attempt
...
//     .object.syntax ("{", "}")
//     .array.syntax  ("[", "]")
var themetext = function themetext()
/* [class, text]+ */
{
  var spans = [];
...
var isempty = function isempty(obj, pl) {
...
function _renderjson(json, indent, dont_indent, show_level, options) {
  ...
  var disclosure = function disclosure(open, placeholder, close, type, builder) {
    var content;
    var empty = span(type);

    var show = function show() {
      if (!content) append(empty.parentNode, content = prepend(builder(), A(options.hide, "disclosure", function () {
    ...
    append(empty, A(options.show, "disclosure", show), themetext(type + " syntax", open), A(placeholder, null, show), themetext(type + " syntax", close));
  ...
  if (typeof json == "string" && json.length > options.max_string_length) return disclosure('"', json.substr(0, options.max_string_length) + " ...", '"', "string", function () {
  ...
  if (isempty(json, options.property_list)) return themetext(null, my_indent, "object syntax", "{}");
